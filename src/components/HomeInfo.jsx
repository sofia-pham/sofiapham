...
const InfoBox = ({ text, link, btnText }) => {
  return (
    <div className="relative sm:text-xl sm:leading-snug text-center bg-green-600 rounded-xl py-4 px-8 text-white mx-5 items-center flex flex-col">
      <p className="font-medium text-center mb-6">{text}</p>
      <Link
        to={link}
        className="absolute bottom-[-15px] bg-white text-green-600 font-semibold px-6 py-2 rounded-lg text-center hover:bg-green-800 hover:text-white shadow-md hover:shadow-lg"
      >
        {btnText}
      </Link>
    </div>
